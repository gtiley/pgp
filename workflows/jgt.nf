...
include { methodsDescriptionText } from '../subworkflows/local/utils_nfcore_jgt_pipeline'

//installed subworkflows from nf-core
...
workflow JGT {
    ...
    contamination_file = Channel.fromPath(params.phix)
    BBMAP_BBDUK (
        fastp_reads,
        contamination_file
    )
...
*/
    ...
    */
    ...
    ch_versions = ch_versions.mix(SELECT_ALL.out.versions.first())
    ...
    ch_versions = ch_versions.mix(SELECT_PASS.out.versions.first())
    ...
    ch_versions = ch_versions.mix(SELECT_BIPASS.out.versions.first())
